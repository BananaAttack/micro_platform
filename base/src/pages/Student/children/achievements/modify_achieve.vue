    ...
    <div v-loading="if_loading">
        ...
        @click="$router.back(-1)"></i>
                ...
                <div style="margin-top: 30px;">
                    <el-form width="500px">
                        ...
                        <el-form-item label="竞赛名称" :label-width="formLabelWidth">
                            <el-select v-model="info.competition_name" placeholder="选择竞赛名称" @change="set_competition" style="width: 400px;">
                        ...
                        <el-form-item label="作品类型" :label-width="formLabelWidth">
                            <el-select v-model="info.workType" @change="$forceUpdate()" placeholder="选择作品类型" style="width: 400px;">
                        ...
                        <el-form-item label="获得奖项" :label-width="formLabelWidth">
                            <el-select v-model="info.award" @change="set_award_value" placeholder="选择奖项" style="width: 400px;">
                    ...
                    <el-button type="primary" @click="goNext" style="margin-top: 30px;margin-left: 300px;width: 120px;">团队成员修改
                    </el-button>
            ...
            <div v-if="achieve_type === '项目'&& !if_members">
                <div style="margin-top: 30px;">
                    ...
                    <el-form-item label="项目名称" :label-width="formLabelWidth">
                        <el-select v-model="info.name"  placeholder="选择项目名称" style="width: 400px;" @change="set_projects_work_type">
                            <el-option v-for="item in projects" :key="item.name" :label="item.name" :value="item.name">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="作品类型" :label-width="formLabelWidth">
                        <el-select v-model="info.work_type" placeholder="选择作品类型" style="width: 400px;">
                            <el-option v-for="(item,i) in projects_type" :key="i" :label="item" :value="item">
                            </el-option>
                        </el-select>
                    </el-form-item>
                ...
                <div style="margin-top: 30px;">
                    <el-form width="500px">
                        ...
                        <el-form-item label="级别" :label-width="formLabelWidth">
                            <el-select v-model="info.name" placeholder="选择论文级别" style="width: 400px;" @change="set_paper_rank">
                    ...
                    <el-button type="primary" @click="goNext" style="margin-top: 30px;margin-left: 300px;width: 120px;" :loading="if_uploading">团队成员修改
                    </el-button>
                    <!-- <el-button class="back-el-button" type="primary" @click="goBack"
                        style="width: 120px;margin-top: 30px;margin-left: 30px;">返回</el-button> -->
                </div>
                ...
                <div style="margin-top: 30px;">
                    <el-form width="500px">
                        ...
                        <el-form-item label="知识产权类型" :label-width="formLabelWidth">
                            <el-select v-model="info.patent_type" placeholder="选择知识产权类型" style="width: 400px;" @change="set_paper_rank">
            ...
            <div v-if="if_members">
                ...
                <el-button type="primary" style="margin-left: 300px;margin-top: 30px;width: 120px;" @click="add_achievement">发送请求
                </el-button>
                <el-button class="back-el-button" type="primary" @click="goBack"
                    style="width: 120px;margin-top: 30px;margin-left: 30px;">返回</el-button>
            </div>
            ...
            <el-dialog title="添加团队成员" :visible.sync="visible_add_members" width="600px" @open="cleanAll_members">
                <el-form @submit.native.prevent>
                    ...
                    <el-form-item label="真实姓名" v-if="if_college" :label-width="formLabelWidth">
                        ...
                        <div v-if="members_selected.length != 0 && members_selected[0].realName!= null" style="height: 300px;overflow: auto;">
            ...
            <el-dialog title="添加指导老师" :visible.sync="visible_add_teachers" width="600px" @open="cleanAll_teachers">
                <el-form @submit.native.prevent>
                    ...
                    <el-form-item label="真实姓名" v-if="if_college" :label-width="formLabelWidth">
                        ...
                        <div v-if="teachers_selected.length != 0 && teachers_selected[0].realName!= null" style="height: 300px;overflow: auto;">
           ...
           async get_patent_single() {
            ...
            goNext() { //进入第二页
                ...
                Object.keys(this.info).forEach(res => {
                    if (this.info[res] === '' && res != 'status'&& res != 'teachers'&& res != 'students' && res != 'achieve_type'&& res != 'type' && res != 'paper_level_value' &&res!='modify' && res != 'pno') {
                        ready_to_go = false
                        console.log(res)
                    }
                })
                ...
                this.axios.get('/opt/achieve/list?choice=1&pageNum=1&pageSize=1000', {
                    ...
                    .then(response => {
                        ...
                        Object.keys(this.projects).forEach(res => {
                            if (this.projects[res].name === this.info.pro_name) {
            ...
            async get_student() { //搜索学生
                if (this.members_single.realName != '') {
                    this.members_selected = await this.$api_user.get_user_by_name(0, this.members_single.realName);
                } else {
            ...
            async get_teacher() { //搜索教师
                if (this.teachers_single.realName != '') {
                    this.teachers_selected = await this.$api_user.get_user_by_name(1, this.teachers_single.realName);
                } else {
            ...
            set_form() { //由于上传表单过于复杂，需要通过次函数构建一个表单,根据成果物类型构建不同的表单
                if (this.info.user_id === '') {
                    ...
                    this.info.year = Number(this.info.achieve_year);
                    if(this.achieve_type === "competition"){
                    this.info.name = this.info.competition_name;
                    }else if (this.achieve_type === "projects") {
                        this.info.year = Number(this.info.pro_date.substr(0, 4));;
                    } else if (this.achieve_type === "paper") {
                        this.info.year = Number(this.info.release_date.substr(0, 4));;
                    } else if (this.achieve_type === "patent") {
                        this.info.year = Number(this.info.grant_date.substr(0, 4));;
                    }else if (this.achieve_type === "荣誉") {
                ...
                this.axios.post('/diff/achieve/student/old', this.info, {
                    headers: {
                        'Token': this.Token
                    },
                })
