<template>
  <div style="display: flex;justify-content: center;align-items: center;flex-direction: column;">
    <div style="width: 100%;min-width: 800px;">
      ...
      <!--                           @change="get_info()"-->
      <!--                           style="display: inline;margin-left: 10px;margin-right: 10px;font-family: helvetica;color: gray;font-weight: lighter;">-->
      <!--                  全选-->
      <!--                </el-radio >-->
      ...
      <div class="table">
        <el-table ref="info_export" :data="info" tooltip-effect="dark" stripe size="small" row-key="achieve_id"
          ...
          <el-table-column label="项目名称" :formatter="if_null" show-overflow-tooltip width="150" align="center">
            <template slot-scope="scope">{{ scope.row.pro_name }}</template>
          </el-table-column>
          <el-table-column prop="category" label="项目类型" :formatter="if_null" show-overflow-tooltip align="center">
          </el-table-column>
          ...
          <el-table-column prop="pro_schedule" label="完成进度" :formatter="if_null" show-overflow-tooltip align="center">
          </el-table-column>
          <el-table-column prop="pro_date" label="立项/结题时间" :formatter="if_null" show-overflow-tooltip align="center"
                           width="150">
          </el-table-column>
  ...
  methods: {
    ...
    get_project_type() { //获取项目作品数据
      this.axios.get('/opt/option/items?type=project_type', {})
        .then(response => {
          this.options.category = response.data.data
    ...
    get_year() {
      ...
      var i = 0;
      for (i; i < 5; i++) {
        this.options.year[y] = y;
        y -= 1;
      }
      this.options.year[y + "及以前"] = y + 10000;
      console.log(this.options.year);
      //this.options.year[i] = y + "及以前";
    },
    //按条件查询
    async get_info() {
      this.selected.start = this.current_page;
      let res = await this.$api_query.admin_search_project(this.selected);
      this.info = res.list;
    ...
    async exprot_which() {
      if (this.export_type == 0) {
        ...
        this.$api_query.admin_export_achieve_single(1, info);
      }
    },
    //设置要下载的条目
    ...
    set_years(e) {
      ...
      this.achieve_year_start = e.target.value;
      if (this.achieve_year_start.length != 0) {
        if (this.achieve_year_start.length != 4) {
