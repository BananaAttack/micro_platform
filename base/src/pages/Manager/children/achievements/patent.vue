<template>
  <div style="display: flex;justify-content: center;align-items: center;flex-direction: column;">
    <div style="width: 100%;min-width: 800px;">
      ...
      <!--                <el-checkbox style="display: inline;margin-right: 10px;color: gray;font-weight: lighter;"-->
      <!--                             :indeterminate="isInd[item.en]" v-model="checkAll[item.en]" @change="HandleCheckAll($event,item.en)">全选-->
      <!--                </el-checkbox>-->
      ...
      <!--                    <el-radio :label="item_2.name" class="achieve_list"-->
      <!--                              style="margin-left: 0px;margin-right: 10px;color: gray;font-weight: lighter;">{{item_2.name}}-->
      <!--                    </el-radio>-->
      <!--                  </el-radio-group>-->
      <!--                </div>-->
      ...
      <!--                              style="margin-left: 0px;margin-right: 10px;color: gray;font-weight: lighter;">{{item_2}}-->
      <!--                    </el-radio>-->
      <!--                  </el-radio-group>-->
      ...
      <!--                &lt;!&ndash; <el-checkbox v-model="selected.user_id" label="" @change="selected.user_id=[],options.name='',get_info()"-->
      ...
      <div class="table">
        <el-table ref="info_export" :data="info" tooltip-effect="dark" stripe size="small" row-key="achieve_id"
                  ...
                  @selection-change="selectionChange" border>
          <el-table-column type="selection" align="center" reserve-selection>
          </el-table-column>
          <el-table-column label="知识产权类型" :formatter="if_null" prop="patent_type" width="150" align="center">
          </el-table-column>
          ...
          <el-table-column prop="patent_number" label="知识产权号" :formatter="if_null" show-overflow-tooltip align="center">
          </el-table-column>
          <el-table-column prop="achieve_year" label="年份" :formatter="if_null" show-overflow-tooltip align="center">
          </el-table-column>
          <el-table-column prop="apply_date" label="申请时间" :formatter="if_null" show-overflow-tooltip align="center">
          </el-table-column>
          <el-table-column prop="grant_date" label="授权时间" :formatter="if_null" show-overflow-tooltip align="center">
          </el-table-column>
  ...
  methods: {
    ...
    get_patent_type() { //获取知识产权类型数据
      this.axios.get('/opt/option/items?type=patent_type', {})
        .then(response => {
          this.options.patent_type = response.data.data
        })
        .catch(
          function (response) {
    ...
    async get_info() {
      ...
      let res = await this.$api_query.admin_search_patent(this.selected);
      this.info = res.list;
    ...
    async exprot_which() {
      if (this.export_type == 0) {
        ...
        this.$api_query.admin_export_achieve_single(3, info);
      }
    },
    //设置要下载的条目
