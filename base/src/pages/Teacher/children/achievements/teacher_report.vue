<template>
 ...
 <div class="header" style="width:100%;margin:30px auto;z-index:20">
     <i class="el-icon-back" style="margin-top: 20px;display: inline-block;margin-right: 20px;font-size: 26px;cursor: pointer;"
      ...
      <el-button type="primary" style="float:right;margin-left:10px;margin-top:10px" @click="getPdf('au_report','认证报告')">
          下载PDF文件
           ...
           <div class="main" id='main_frame' style="background:#909399;width:100%;border:1px solid black;margin:10px auto;height: auto;overflow:hidden" >
         <div id="au_report" style="width:900px;background:white;margin:40px auto;padding:40px;overflow:hidden">
            <div class="head" style="width:95%;height:235px;border-bottom:1px solid #909399;margin: auto">
              ...
              <div style="font-size:40px;font-weight:bolder;float:left;height:60px">科技指导认证报告</div>
                ...
                <div class="web_info" style="width:39%;height:60px;float:left;margin-left:10px;">
                    <p style="line-height:33px;font-size:12px">验证码:{{$route.params.id}}</p>
                    <p style="line-height:18px;font-size:12px">查询网站:124.222.44.159/#/check_report</p>
                </div>
          </div>
          <div class="detail_info" style="width:100%;height:170px;">
          <div style="width:40%;height:auto;float:left;margin:20px 0px">
              <p style="font-size:20px;line-height:30px;">姓名：{{teacher_info.realName}}
              </p>
               <p style="font-size:20px;line-height:35px;">
                   所属院系：{{teacher_info.department}}
              </p>
               <p style="font-size:20px;line-height:35px;">
                   入职年份：{{teacher_info.entry_year}}
              </p>
               <p style="font-size:20px;line-height:35px;font-weight:bolder">
                   本报告指导总分：{{sum}}分
              </p>
         ...
         <div style="width:40%;height:auto;float:left;margin:20px 0px">
              <p style="font-size:20px;line-height:30px;">职称：{{teacher_info.ranks}}
              </p>
               <p style="font-size:20px;line-height:35px;">
                   手机号：{{teacher_info.phone}}
              </p>
               <p style="font-size:20px;line-height:35px;">
                   邮箱：{{teacher_info.email}}
              </p>
         </div>
           ...
           <div class="photo" style="height:220px;width:18%;float:right">  <img :src=teacher_info.picture width="100%" height="100%"></div>
      </div>  
          <div class="tabelframe" style="width:100%;height:auto;margin-top:30px" id="tabel1">
           <p style="font-size:25px;font-weight:bolder">竞赛指导成果 {{competitions_list.length}}项 合计{{competition_sum}}分</p>
           <el-table
           ref="tabel2"
 ...
 <el-table-column
           ...
           <p style="font-size:25px;font-weight:bolder">项目指导成果 {{projects_list.length}}项 合计{{project_sum}}分</p>
           <el-table
          :row-style="{borderColor:'black'}"
           :cell-style="{borderColor:'black'}"
           border
 ...
 <el-table-column
           ...
           <p style="font-size:25px;font-weight:bolder">论文指导成果 {{papers_list.length}}项 合计{{paper_sum}}分</p>
           <el-table
           :row-style="{borderColor:'black'}"
           :cell-style="{borderColor:'black'}"
            border
 ...
 <el-table-column
           ...
           <p style="font-size:25px;font-weight:bolder">知识指导产权 {{patents_list.length}}项 合计{{patent_sum}}分</p>
           <el-table
             :row-style="{borderColor:'black'}"
           :cell-style="{borderColor:'black'}"
           border
  ...
  if(res.data.code===2000){
     ...
     this.axios.get("http://124.222.44.159:8080/user/diff/achieve",{
      ...
      if(Response.data.code===2000){
        ...
        this.competition_sum+=Response.data.data.award_value
        if(Response.data.data.award_value!=null)
        this.sum+=Response.data.data.award_value
      }else{
     ...
     for(var i=0;i<eval(res.data.data.papers).length;i++){
      ...
      if(Response.data.code===2000){
        ...
        this.parent_sum+=Response.data.data.award_value
        if(Response.data.data.award_value!=null)
          this.sum+=Response.data.data.award_value
      }else{
        console.log(Response)
     ...
     for(var i=0;i<eval(res.data.data.projects).length;i++){
      ...
      if(Response.data.code===2000){
        ...
        this.project_sum+=Response.data.data.award_value
        if(Response.data.data.award_value!=null)
        this.sum+=Response.data.data.award_value
      }else{
        console.log(Response)
    ...
    this.axios.get("http://124.222.44.159:8080//user/teacher/info",{
      params:{
        n:0,
        user_id:this.$store.getters.user_id
      }
